<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Thêm Phiếu Dịch Vụ</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</head>
<body class="bg-light">
    <div class="container d-flex justify-content-center align-items-center min-vh-100">
        <div class="card shadow-lg p-4" style="width: 100%; max-width: 500px;">
            <div class="card-body">
                <h1 class="text-center text-primary mb-3">Thêm Phiếu Dịch Vụ</h1>
                <p class="text-center text-muted">Vui lòng điền thông tin phòng</p>
                <form action="{{ route('phieudichvu.store') }}" method="POST">
                    @csrf
                   
                    <div class="mb-3">
                        <label for="MaPhieuDV" class="form-label">Mã Phiếu Dịch Vụ</label>
                        <input type="text" class="form-control" id="MaPhieuDV" name="MaPhieuDV" placeholder="VD: PDV001" required>
                    </div>

                    <div class="mb-3">
                        <label for="NgayLap" class="form-label">Ngày Lập</label>
                        <input type="date" class="form-control" id="NgayLap" name="NgayLap" required>
                    </div>
                    
                    <div class="mb-3">
                        <label for="MaPT" class="form-label">Mã Phiếu Thuê</label>
                        <select class="form-select" id="MaPT" name="MaPT" required>
                            <option value="" disabled selected>Chọn phiếu thuê</option>
                            @foreach ($phieuthues as $phieuthue)
                                <option value="{{ $phieuthue->MaPT }}">
                                    {{ $phieuthue->MaPT }} - {{ $phieuthue->MaPhong }} - {{ number_format($phieuthue->GiaMotNgay, 0, ',', '.') }} VND
                                </option>
                            @endforeach
                        </select>
                    </div>
                    
                    <!-- Nút hành động -->
                    <div class="d-flex justify-content-between">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save"></i> Lưu
                        </button>
                        <a href="{{ route('phieudichvu.index') }}" class="btn btn-secondary">
                            <i class="bi bi-arrow-left"></i> Hủy
                        </a>
                    </div>
                </form>
            </div>
        </div>
    </div>
</body>
</html>
